...
<script>
import Header from "@/components/Advertiser/Includes/Header";
import SideNav from "@/components/Advertiser/Includes/SideNav";
...
export default {
  ...
  watch: {
    $route: {
        immediate: true,
        handler(to, from) {
            if (!this.$store.state.auth.auth_token || this.$store.state.auth.auth_instance.role != 'advertiser') {
